...
//    
//    // 임시 오버라이드 시스템
//    @State private var temporaryOverride: LiveActivityType? = nil
//    @State private var overrideTimer: Timer? = nil
//    
//    enum LiveActivityType {
//        case timer, charging, volume, music
//    }
//    
//    // Volume Icon 계산
//    private var volumeIconName: String {
...
//            extractColor(from: newAlbumArt)
//        }
//        .onChange(of: volumeManager.isVolumeHUDVisible) { _, isVisible in
//            if isVisible {
//                showTemporaryOverride(.volume, duration: 2.0)
//            }
//        }
//        .onChange(of: chargeDetectManager.isHUDActive) { _, isActive in
//            if isActive {
//                showTemporaryOverride(.charging, duration: 3.0)
//            }
//        }
//        .onAppear {
//            extractColor(from: musicManager.albumArt)
...
//    
//    // MARK: - 임시 오버라이드 시스템
//    private func showTemporaryOverride(_ type: LiveActivityType, duration: TimeInterval = 2.0) {
//        temporaryOverride = type
//        
//        overrideTimer?.invalidate()
//        overrideTimer = Timer.scheduledTimer(withTimeInterval: duration, repeats: false) { _ in
...
//                        // 임시 오버라이드가 있으면 그것을 우선 표시
//                        switch override {
//                        case .volume: volumeLiveActivity
//                        case .charging: chargingLiveActivity
//                        case .timer: timerLiveActivity
//                        case .music: musicLiveActivity
//                        }
//                    } else {
//                        // 기본 우선순위
//                        if timerManager.isTimerHUDVisible {
...
//    private func calculateNotchWidth() -> CGFloat {
//        if vm.notchState == .off {
//            // 임시 오버라이드가 있으면 해당 크기 사용
//            if let override = temporaryOverride {
//                switch override {
//                case .timer: return vm.notchSize.width + 90
//                case .charging: return vm.notchSize.width + 95
//                case .volume: return vm.notchSize.width + 110
//                case .music: return vm.notchSize.width + 60
//                }
//            }
//            
//            // 기본 우선순위에 따른 크기
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: vm.notchState) {
            print("노치 상태 변경: \(vm.notchState)")
            // .off 사용
            if vm.notchState == .off && isHovering {
                print("노치 닫힘 - hover 상태 리셋")
        ...
        .onChange(of: fullscreenDetector.shouldHideNotch) { _, shouldHide in
            // 전체화면 감지 시 상태 리셋
            if shouldHide {
                hoverTask?.cancel()
                debounceWorkItem?.cancel()
    ...
    // MARK: - 개선된 호버 처리 함수
    private func handleHover(hovering: Bool) {
        if isHoverStateChanging {
            print("상태 변경 중 - hover 이벤트 무시")
            return
        }
        ...
        if hovering {
            ...
            // .off 사용 (NotchStatus.off)
            if !hasTriggeredHaptic && vm.notchState == .off {
                print("햅틱 피드백 트리거")
                hapticFeedback.toggle()
            ...
            let task = DispatchWorkItem {
                // .off 사용
                guard vm.notchState == .off,
                      isHovering,
                      !isHoverStateChanging else {
                    print("노치 열기 조건 불만족")
                    return
                }
                
                DispatchQueue.main.async {
                    print("0.4초 후 노치 자동 열기 실행")
                    withAnimation(.spring(response: 0.6, dampingFraction: 0.8)) {
                        vm.open()
                    }
                }
